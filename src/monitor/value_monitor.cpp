...
bool readValue(task_t task, mach_vm_address_t address, std::size_t size, std::vector<std::uint8_t>& buffer)
{
    buffer.resize(size);
...
bool ValueMonitor::addAddress(mach_vm_address_t address, std::size_t size)
{
    ...
    if (addresses_.size() >= max_addresses_) return false;
...
void ValueMonitor::removeAddress(mach_vm_address_t address)
{
    ...
    addresses_.erase(std::remove_if(addresses_.begin(), addresses_.end(),
        [address](const MonitoredAddress& e) { return e.address == address; }), addresses_.end());
}
...
std::vector<ValueMonitor::ValueChange> ValueMonitor::poll(task_t task)
{
    ...
    if (task == MACH_PORT_NULL) return changes;

    for (auto& entry : addresses_) {
        std::vector<std::uint8_t> current;
        if (!readValue(task, entry.address, entry.value_size, current)) continue;
...
std::vector<ValueMonitor::MonitoredAddress> ValueMonitor::tracked()
{
    std::lock_guard<std::mutex> lock(mutex_);
    return addresses_;
}
